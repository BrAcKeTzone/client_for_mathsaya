  ...
  if (selectedYunitId) {
    try {
      const response = await axios.get(
        `${server_url}/lessons/${selectedYunitId}`
      );
      const lessons = response.data;

      lessons.forEach((lesson) => {
        LessonThumbnailMapping[lesson.lessonName] = lesson.lessonThumbnail;
        lessonIdMapping[lesson.lessonName] = lesson.lessonId;
      });
...
export default class LessonsChoices extends Phaser.Scene {
  ...
  async create() {
    ...
    for (const lessonName in LessonThumbnailMapping) {
      const thumbnailUrl = LessonThumbnailMapping[lessonName];
      this.load.image(`yunit_${lessonName}`, thumbnailUrl);
    }

    this.displayBackButton(audioState);
    this.displaySceneName();
  ...
  createLessonImages(LessonThumbnailMapping, lessonIdMapping, audioState) {
    const sortedLessons = Object.entries(LessonThumbnailMapping)
      .map(([lessonName, thumbnailUrl]) => {
        return { lessonName, thumbnailUrl };
      })
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const { lessonName, thumbnailUrl } = lesson;
      const y = (i - startIndex + 1) * spacing;
      const x = this.game.config.width / 2;

      const image = this.add.image(x, y, `yunit_${lessonName}`);
      image.setScale(imageSize / image.width, imageSize / image.height);

      const titleLabel = this.add.text(x + imageSize / 2 + 10, y, lessonName, {
        fontFamily: "Comic Sans MS",
        fontSize: "20px",
        fill: "#ffffff",
        backgroundColor: "#333333",
      ...
      image.on("pointerdown", () => {
        if (this.selectedButton === image) {
          ...
          image.setDepth(5);
          const lessonId = lessonIdMapping[lessonName];
          this.tweens.add({
            targets: image,
            angle: 360,
            scaleX: 4,
            ...
            onComplete: () => {
              ...
              console.log(`Clicked on Lesson ${lessonName}`);
              this.cameras.main.fadeOut(500);
              this.cameras.main.on("camerafadeoutcomplete", () => {
