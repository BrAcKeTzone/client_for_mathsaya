...
async function fetchStudentProfile() {
  ...
  try {
    ...
    completedExercises = studentData.completedExercises;
...
export default class ExercisesChoices extends Phaser.Scene {
  ...
  displayExercises(page, audioState) {
    // Sort the exercises by exerciseName
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      exerciseLabel.on("pointerover", () => {
        ...
        const { exerciseName, exerciseDescription } = exercise;
        const textToSpeak = `${exerciseName}. ${exerciseDescription}`;
        if (audioState === "On") {
      ...
      exerciseLabel.on("pointerdown", () => {
        if (this.selectedButton === exerciseLabel) {
          ...
          if (!isCompleted) {
            ...
            const { exerciseId, exerciseName } = selectedExercise;
            this.tweens.add({
              ...
              onComplete: () => {
                ...
                console.log(`Clicked on Exercise ${exerciseName}`);
                this.cameras.main.fadeOut(500);
                this.cameras.main.on("camerafadeoutcomplete", () => {
