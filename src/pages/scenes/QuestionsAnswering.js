  ...
  if (selectedExerciseId) {
    try {
      ...
      const questions = response.data.map((question) => ({
        ...question,
        answer_choices: question.answer_choices.split(","), // Convert string to array
      }));
...
export default class QuestionsAnswering extends Phaser.Scene {
  ...
  async create() {
    ...
    questions.forEach((question) => {
      if (question.questionImage) {
        this.load.image(question.questionId, question.questionImage);
      }
    });
  ...
  createContainers() {
    questionContainer = this.add.container(
      ...
      this.game.config.height / 8 // Adjusted to the middle top
    );
