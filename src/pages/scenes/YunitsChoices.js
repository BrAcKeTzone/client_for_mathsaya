  ...
  if (teacherId) {
    try {
      ...
      yunits.forEach((yunit) => {
        yunitThumbnailMapping[yunit.yunitName] = yunit.yunitThumbnail;
        yunitIdMapping[yunit.yunitName] = yunit.yunitId;
      });
...
export default class YunitsChoices extends Phaser.Scene {
  ...
  async create() {
    ...
    for (const yunitName in yunitThumbnailMapping) {
      const thumbnailUrl = yunitThumbnailMapping[yunitName];
      this.load.image(`yunit_${yunitName}`, thumbnailUrl);
    }
  ...
  createYunitChoices(yunitThumbnailMapping, yunitIdMapping, audioState) {
    const sortedYunits = Object.entries(yunitThumbnailMapping)
      .map(([yunitName, thumbnailUrl]) => {
        return { yunitName, thumbnailUrl };
      })
      .sort((a, b) => a.yunitName.localeCompare(b.yunitName));
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const image = this.add.image(x, y, `yunit_${yunit.yunitName}`);
      ...
      image.on("pointerover", () => {
        ...
        if (audioState === "On") {
          responsiveVoice.speak(yunit.yunitName, "Filipino Female");
        }
      });
      ...
      image.on("pointerdown", () => {
        if (this.selectedButton === image) {
          ...
          image.setDepth(5);
          const yunitId = yunitIdMapping[yunit.yunitName];
          this.tweens.add({
            targets: image,
            angle: 360,
            scaleX: 4,
            ...
            onComplete: () => {
              ...
              console.log(`Clicked on Yunit ${yunit.yunitName}`);
              this.cameras.main.fadeOut(500);
              this.cameras.main.on("camerafadeoutcomplete", () => {
